 ...
 *
 * B is superior to A as it returns a UNIQUE and EXPLICIT exception indicating abnormal behaviour.
 * A does not make note of any withdrawal problems in its design (rather returning -1) which is an
 * non-explicit "error-state" which can be mistaken as normal behaviour. B can be later be made to
 * handle more robust situations, and is direct and explicit in noting problems within withdrawal.
 * A creates further design problems later-on as one has to deal with the return value of -1 and
